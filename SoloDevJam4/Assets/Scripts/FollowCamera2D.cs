...
namespace CustomCamera
{
    ...
    public class FollowCamera2D : MonoBehaviour
    {
        ...
        private static FollowCamera2D instance;
        ...
        public static void SetTarget(Transform targetTransform)
        {
            if (instance != null)
            {
                instance.target = targetTransform;
            }
        }

        void LateUpdate ()
        {
            // If the character is dead, reset cam to position zero.
            if (isDead)
            {
                Vector3 targetPosition = new Vector3(0,0,-10);
                transform.position = Vector3.SmoothDamp(transform.position, targetPosition, ref velocity, dampTime/5);
                return;
            }
